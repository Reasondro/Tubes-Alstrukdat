            ...
            if (!(idxl <= LPenyu.Penyanyi[idxp].album.AlbumKe[idxa].DaftarLagu.JumlahLagu && idxl > 0)) {
                ...
                if (!(idxplay <= DP.Neff && idxplay > 0)) {
                    ...
                    if (Search(DP.pl[idxplay], Info(P))) {
                        ...
                        stringCopy(X.judul_lagu.judul, LPenyu.Penyanyi[idxp].album.AlbumKe[idxa].DaftarLagu.Songs[idxl].judul);
    ...
    if (!foundp) {
        ...
        if (!founda) {
            ...
            if (!(idxplay <= DP.Neff && idxplay > 0)) {
                ...
                int count = 0, idxl = 0;
                address P;
                while (idxl < LPenyu.Penyanyi[idxp].album.AlbumKe[idxa].DaftarLagu.JumlahLagu) {
                    P = Alokasi(penyanyi, album, LPenyu.Penyanyi[idxp].album.AlbumKe[idxa].DaftarLagu.Songs[idxl].judul);
                    if (!Search(DP.pl[idxplay], Info(P))) {
